...
covid %>%
  filter(date == max(date)) %>%
  group_by(state) %>%
  summarize(currentCases = sum(cases, na.rm = TRUE)) %>%
  arrange(-currentCases) %>%
  head(5)
```
...
covid %>%
  filter(date == max(date)) %>%
  mutate(deathRatio = 100*(deaths/cases)) %>%
  filter(deathRatio != Inf) %>% # Handle dividing by 0
  arrange(-deathRatio) %>%
  head(5)
```
...
covid %>%
  filter(date == max(date)) %>%
  group_by(state) %>%
  summarize(currentCases = sum(cases, na.rm = TRUE), currentDeaths = sum(deaths, na.rm = TRUE)) %>%
  mutate(deathRatio = 100*(currentDeaths/currentCases)) %>%
  filter(deathRatio != Inf) %>% # Handle dividing by 0
  arrange(-deathRatio) %>%
  head(5)
```
